...
public class PathSumII {
    ...
    public static List<List<Integer>> pathSum(TreeNode root, int sum) {
        ...
        helper(root, sum-root.val, currList, res);
        return res;
    }

    private static void helper(TreeNode root, int sum, List<Integer> currList, List<List<Integer>> res) {
        if (root == null) return;
        ...
        if (root.left != null) {
            ...
            helper(root.left, sum-root.left.val, currList, res);
            currList.remove(currList.size()-1);
        }
        if (root.right != null) {
            currList.add(root.right.val);
            helper(root.right, sum-root.right.val, currList, res);
            currList.remove(currList.size()-1);
        }
    }
